...
from .nodepertuber import NodePerturber
...
class CFNodeExplainer(RecourseMethod):
    """
    ...
    def __init__(self, mlmodel: GAT, data: DataCatalog, hyperparams: Dict = None):
        ...
        super(CFNodeExplainer, self).__init__(mlmodel=mlmodel)
        self.data = data
    ...
    def train(self, epoch: int, graph: Data, oracle):
        ...
        if self.model_type == "gnn" or self.model_type == "gat" or self.model_type == "gin":
        ...
        elif self.model_type == "gnn_coo" or self.model_type == "gat_coo" or self.model_type == "gin_coo":
            edge_ind = dense_to_sparse(graph.adj)
            differentiable_output = self.cf_model.forward_coo(graph.x, edge_ind, graph.edge_attr) 
            model_out, V_pert, P_x = self.cf_model.forward_prediction_coo(graph.x, edge_ind, graph.edge_attr) 
    ...
    def get_counterfactuals(self, factuals: Union[str, pd.DataFrame]):
        ...
        adj = df_test.create_adj_matrix(data_graph).squeeze()  # Does not include self loops
        ...
        if self.model_type == "gnn" or self.model_type == "gat" or self.model_type == "gin":
            ...
            edge_index, edge_attr = data_graph.edge_index.to(self.device), data_graph.edge_attr.to(self.device)
            y_pred_orig = self.mlmodel.predict_gnn_coo(features, edge_index, edge_attr)
        ...
        for i in idx_test[:]:
            ...
            if cf_example is None:
                ...
                numerator_sparsity+=node_sparsity(factual, cf_example)
...
def plotting(num_epochs, validity, sparsity, fidelity, model_type):
    ...
    # Plot della loss
    plt.subplot(3, 1, 1)
    plt.plot(range(1, num_epochs+1), validity , label='Validity')
    plt.xlabel('Num. Counterfactual')
    plt.ylabel('Validity')
    plt.title('Validity Evaluation')
    plt.legend()

    # Plot dell'accuracy
    plt.subplot(3, 1, 2)
    plt.plot(range(1, num_epochs+1), sparsity , label='Sparsity')
    plt.xlabel('Num. Counterfactual')
    plt.ylabel('Sparsity')
    plt.title('Sparsity Evaluation')
    plt.legend()

    # Plot dell'F1-score
    plt.subplot(3, 1, 3)
    plt.plot(range(1, num_epochs+1), fidelity , label='Fidelity')
    plt.xlabel('Num. Counterfactual')
    plt.ylabel('Fidelity')
    ...
    # Salvare l'immagine
    plt.tight_layout()
    path = '/home/hl-turing/VSCodeProjects/Flavio/CARLA/carla/images_plotting/cf_results_with_nodepert_' + model_type+ '.png'
